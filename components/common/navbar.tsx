    ...
    return (
        <nav className={`${isSidebarOpen ? 'w-full flex justify-between items-center h-[80px] sm:h-[98.19px]' : 'w-full flex justify-between items-center h-[80px] sm:h-[98.19px]'}`} >
            <div className='meba-box__flex-between'>
                <div className='flex sm:ml-1 ml-10 mr-5 sm:mr-5 items-center justify-center'>
                    <button className=' hover:bg-neutral-300/90 active:scale-110  dark:hover:bg-neutral-500/80  text-center p-[0.5rem] transition border-gray hover:outline-none rounded-full' onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
                        <MenuIcon />
                    </button>
                </div>
